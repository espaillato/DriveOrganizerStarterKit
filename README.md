...
2. **Living Reference** — a small set of Obsidian notes, one dashboard per life domain (Health, Finance, Legal, Identity, whatever you add), each readable in a couple of minutes: current state, active strategy, things to watch. Deep analysis and history live in linked `_Reference/` subnotes, not in the dashboard itself. If you're on Google Drive and want to read these straight from the browser without opening Obsidian, the [Google Drive Markdown Preview](https://chromewebstore.google.com/detail/google-drive-markdown-pre/mcggigjpaggcpngfcgcdconaijliicda) Chrome extension renders `.md` files properly instead of showing raw text — see `SETUP_GUIDE.md` step 3.
